...
- **GOTCHA (fixed in runtime v0.12.2):** the route wrapper used `req.body || req.query`, and Express sets `req.body` to `{}` for GET, so every GET query param (`source`/`fit`/`baseline`/`score`) was silently dropped. v0.12.2 merges them. This Node now pins `#v0.12.2`; a box deploy MUST refresh the runtime (`rm -rf node_modules/@developai && npm install`) or these features stay broken.
