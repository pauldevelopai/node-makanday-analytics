...
| **Decision history** (`lib/history.js`; `GET/DELETE /api/history[/:id]`; History tab) | Keep every brief / recommendation / idea-set + a dated audience snapshot, in `host.store` (uncapped, local + hosted), reviewable so decisions build on the past | Save/list/get/delete + snapshot + the `postBrief` hook unit-tested (fake host); History tab rendered live locally — 3 seeded entries (newest-first, kind badges, snapshot omitted from list, Open fetches full body + the audience picture it was based on, Delete works). Live generation-writes-history not yet run with a real key | ✅ | 2026-07-20 |
