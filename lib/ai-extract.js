...
const INPUT_CHAR_CAP = 120000; // ~30k tokens

export async function aiExtractRows(host, text) {
  const full = String(text || "");
  const input = full.slice(0, INPUT_CHAR_CAP);
  ...
  const result = parseAiTsv(out);
  // Don't let a large file silently lose its tail: flag the truncation so it
  // shows up in the data-quality report instead of looking like a clean ingest.
  if (full.length > INPUT_CHAR_CAP) {
    result.issues.unshift({
      n: 0, level: "warn", field: "file",
      msg: `This document is very large — only the first ~${Math.round(INPUT_CHAR_CAP / 1000)}k characters were read, so some later stories may be missing. Split it into smaller files to capture everything.`
    });
  }
  return result;
}
