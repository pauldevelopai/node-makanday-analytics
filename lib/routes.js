...
import { recommend, storyIdeas } from './handlers.js';
import { listHistory, getHistoryEntry, deleteHistoryEntry } from './history.js';
...
export function mountAnalyticsRoutes(app, getHost) {
  ...
  // ── AI story ideas: commissionable pitches from proven audience appetite ──
  app.post('/api/ideas', wrap(async (req, host) => storyIdeas(host, req.body || {})));

  // ── Decision history: every brief/recommendation/idea-set + dated audience snapshots ──
  app.get('/api/history', wrap(async (req, host) => ({ ok: true, history: await listHistory(host, req.query || {}) })));
  app.get('/api/history/:id', wrap(async (req, host) => {
    const entry = await getHistoryEntry(host, req.params.id);
    return entry ? { ok: true, entry } : { ok: false, message: 'not found' };
  }));
  app.delete('/api/history/:id', wrap(async (req, host) => deleteHistoryEntry(host, req.params.id)));
