...
// Map a header name to one of our fields. First matching pattern wins.
...
export function csvToMatrix(text) {
  ...
  for (let r = 1; r < table.length; r++) {
    ...
    const title = String(cells[col.title] || "").replace(/https?:\/\/\S+/g, "").trim();
    ...
    if (reach !== null && engagement !== null && title) {
      rows.push({ n, title, month, date: dateRaw, reach, engagement, type });
    }
  }
