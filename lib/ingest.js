...
import { csvToMatrix } from "./csv.js";
import { setExtras, setSourceMeta } from "./store.js";
...
export function parseMatrixHtml(html) {
  ...
  for (const tr of trs) {
    ...
    // Capture a story link if the description carries one, BEFORE stripping it out.
    const url = (cells[1].match(/https?:\/\/\S+/) || [])[0] || null;
    ...
    if (reach !== null && engagement !== null) {
      rows.push({ n, title, month, date, reach, engagement, type, url });
    }
  }
 ...
 * @param {{kind?:string, sheetUrl?:string}} opts  source provenance (sheet/file)
 */
export async function ingestMatrix(host, buffer, sourceLabel, opts = {}) {
  ...
  if (fmt === "unknown") {
    throw new Error("Unsupported file. Upload a Word document (.docx/.doc), a PDF, a spreadsheet (.csv), or paste a Google Sheet link.");
  }

  let rows, issues;
  if (fmt === "csv") {
    // Spreadsheet / Google Sheet export — deterministic, free, no AI needed.
    ({ rows, issues } = csvToMatrix(buffer.toString("utf8")));
  } else if (fmt === "docx") {
  ...
  // Per-story enrichment overlay (url + any readership the source carried) — kept
  // in host.store, NOT the stories table, so post-ingest edits (links, scrape)
  // work on the lite host too. Replace-ingest overwrites the whole source's map.
  const extras = {};
  for (const r of rows) {
    const e = {};
    if (r.url) e.url = r.url;
    if (r.unique_readers != null) e.unique_readers = r.unique_readers;
    if (r.pageviews != null) e.pageviews = r.pageviews;
    if (Object.keys(e).length) extras[r.n] = e;
  }
  await setExtras(host, sourceLabel, extras).catch(() => {});

  // Source provenance — drives the "manage sources" panel + Google-Sheet refresh.
  // Only set keys we actually have, so a refresh (re-ingest) preserves an existing
  // sheet_url/kind via setSourceMeta's merge instead of nulling them.
  const metaPatch = { row_count: rows.length, last_refreshed: new Date().toISOString() };
  if (opts.sheetUrl) { metaPatch.sheet_url = opts.sheetUrl; metaPatch.kind = "sheet"; }
  else if (opts.kind) metaPatch.kind = opts.kind;
  await setSourceMeta(host, sourceLabel, metaPatch).catch(() => {});
