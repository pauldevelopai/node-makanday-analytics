...
import { wilson, mannWhitney } from "./stats.js";
import { wordSignal } from "./wordsignal.js";
import { sentimentSignal } from "./sentiment.js";
import { byWeekday, quadrants, performanceOutliers } from "./insights.js";
...
export function enrich(rows, beats = BEATS) {
  ...
  return rows.map(r => {
    ...
    // Wilson interval on the rate (engagement out of reach): wlb is the
    // sample-size-aware ranking score; ciLo/ciHi are the displayed % bounds.
    const ci = wilson(r.engagement || 0, r.reach || 0);
    return {
      ...r,
      rate,
      band,
      wlb: ci.lo,
      ciLo: +(ci.lo * 100).toFixed(2),
      ciHi: +(ci.hi * 100).toFixed(2),
...
// Median rate + reach for a group, plus a significance flag: do this group's
// per-story rates really differ from the rest? (Mann-Whitney on the rates, so
// the sample is the story count — not impressions.)
function groupStats(group, rest) {
  return {
    n: group.length,
    medianRate: +median(group.map(d => d.rate)).toFixed(2),
    medianReach: Math.round(median(group.map(d => d.reach))),
    significant: mannWhitney(group.map(d => d.rate), rest.map(d => d.rate)).significant
  };
}
...
export function byBeat(data) {
  return beatNamesIn(data)
    .map(b => {
      ...
      return { beat: b, ...groupStats(s, data.filter(d => !d.beats.includes(b))) };
    })
    .filter(Boolean)
...
export function byFormat(data) {
  ...
  return types
    .map(t => ({ type: t, ...groupStats(data.filter(d => d.type === t), data.filter(d => d.type !== t)) }))
    .sort((a, b) => b.medianRate - a.medianRate);
}
...
export function headlineSignal(data) {
  ...
  return keys.map(k => {
    ...
    return {
      ...
      significant: mannWhitney(yes.map(d => d.rate), no.map(d => d.rate)).significant
    };
  });
}
...
// Reach floor keeps editorially-marginal tiny posts out of the lead (a story
// that reached 30 people isn't a "win" to emulate); WITHIN that, rank by the
// Wilson lower bound so a high rate proven over a big audience beats a high rate
// from a thin sample. Flukes are demoted, never hidden.
export function signalLeaders(data, n = 15) {
  const floor = reachFloor(data);
  const byWlb = (a, b) => b.wlb - a.wlb;
  const qualified = data.filter(d => d.reach >= floor).sort(byWlb);
  const rest = data.filter(d => d.reach < floor).sort(byWlb);
...
export function fullReport(rows, beats = BEATS) {
  ...
  return {
    ...
    wordSignal: wordSignal(data),
    sentiment: sentimentSignal(data),
    weekday: byWeekday(data),
    quadrants: quadrants(data),
    outliers: performanceOutliers(data),
    stories: data
  };
}
